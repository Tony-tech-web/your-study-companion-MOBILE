...
const OrbitSplashWord = () => (
  <Svg width={184} height={72} viewBox="0 0 184 72" fill="none">
    <Defs>
      <LinearGradient id="splashWord" x1="0" y1="8" x2="178" y2="60" gradientUnits="userSpaceOnUse">
...
function RootLayoutNav() {
  ...
  const wordTranslate = useRef(new Animated.Value(-46)).current;
  ...
  }, [session, loading, segments, router, logoScale, markScale, splashOpacity, wordOpacity, wordTranslate]);
    ...
    <>
      ...
      {showSplashIntro && (
        <Animated.View pointerEvents="none" style={[splashStyles.overlay, { opacity: splashOpacity }]}>
          <Animated.View style={[splashStyles.logo, { transform: [{ scale: logoScale }] }]}>
            <Animated.View style={{ transform: [{ scale: markScale }] }}>
              <OrbitSplashMark />
            </Animated.View>
            <Animated.View style={[splashStyles.word, { opacity: wordOpacity, transform: [{ translateX: wordTranslate }] }]}>
              <OrbitSplashWord />
            </Animated.View>
          </Animated.View>
        </Animated.View>
